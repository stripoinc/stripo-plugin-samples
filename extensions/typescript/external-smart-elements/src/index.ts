...
// Declare UIEditor on the window object for TypeScript
declare global {
    interface Window {
        UIEditor: {
...
function _runEditor(template: Template, extension: unknown): void {
    window.UIEditor.initEditor(
        document.querySelector('#stripoEditorContainer') as HTMLElement,
        {
            html: template.html,
            css: template.css,
            metadata: {
...
function _request(method: string, url: string, data: string | null, callback: (responseText: string) => void): void {
